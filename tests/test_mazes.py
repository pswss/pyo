...
def test_demo_maze_shapes():
    maze = demo_maze()
    assert len(maze["walls"]) == 19           # 세그먼트 여러 개
    assert len(maze["free_cells"]) == 20     # 자유 타일 여러 개
...
def test_traversal_visits_all_cells_with_small_steps():
    ...
    # 경로가 벽을 관통하지 않음
    from harness.mazes import _crosses_wall
    for i in range(1, len(path)):
        assert not _crosses_wall(tuple(pts[i - 1]), tuple(pts[i]), maze["walls"]), i
    # 개구부 통과 확인: y=2*TILE 경계를 x∈(2.4T, 2.9T) 구간에서 건넘
    crossed_gap = any(
        pts[i - 1][1] < 2 * TILE <= pts[i][1]
        and 2.4 * TILE < pts[i][0] < 2.9 * TILE
        for i in range(1, len(path))
    )
    assert crossed_gap, "개구부로 통과하는 경로 없음"
